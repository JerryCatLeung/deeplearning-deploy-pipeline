...
def model_fn(features, labels, mode, params):
    ...
    # ------bulid weights------
    Feat_Emb = tf.get_variable(name='embeddings', shape=[feature_size, embedding_size],
        ...
        if FLAGS.attention_pooling:
            def attention_unit(Feat_Emb, sp_ids, sp_weights, a_xx_emb):
                ...
                dense_mask = tf.expand_dims(tf.cast(dense_ids > 0, tf.float32),
                                            axis=-1)  # None * P * 1     0=padding >0取非padding
                ...
                ax_emb = tf.reshape(tf.tile(a_xx_emb, [1, padded_dim]), shape=[-1,
                                                                               embedding_size])  # None * K --> (None * P) * K     注意跟dense_emb reshape顺序保持一致
                x_inputs = tf.concat([ub_ebm, ub_ebm - ax_emb, ax_emb], axis=1)  # (None * P) * 3K
                for i in range(len(attention_layers)):
                    x_inputs = tf.contrib.layers.fully_connected(inputs=x_inputs, num_outputs=layers[i],
                                                                 scope='att_fc%d' % i)
                    if FLAGS.batch_norm:
                ...
                att_wgt = tf.contrib.layers.fully_connected(inputs=x_inputs, num_outputs=1, activation_fn=tf.sigmoid,
                                                            scope='att_out')  # (None * P) * 1
            ...
            u_cat_emb = tf.nn.embedding_lookup_sparse(Feat_Emb, sp_ids=u_catids, sp_weights=u_catvals,
                                                      combiner="sum")  # None * K
    ...
    with tf.variable_scope("MLP-layer"):
        ...
        x_deep = tf.concat(
            [tf.reshape(common_embs, shape=[-1, common_dims]), u_cat_emb, u_shop_emb, u_brand_emb, u_int_emb, a_cat_emb,
             a_shop_emb, a_brand_emb, a_int_emb], axis=1)  # None * (F*K)
